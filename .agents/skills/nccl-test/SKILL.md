---
name: nccl-test
description: Run NCCL allreduce bandwidth test on 2 remote GPU nodes via SSH and capture quick deploy/debug signals
user-invocable: true
allowed-tools: Bash
...
Run the torch_mnist_train.py allreduce benchmark on two remote GPU nodes.
...
- SSH user is `modal`
- If a worker hostname fails, also try the `-1` fallback form before giving up.
...
After pulling, verify the deployed commit explicitly:
```
ssh modal@<node> 'cd ~/gvisor && git rev-parse HEAD'
```
...
### Optional: Fast debug triage
For quick RDMA triage, also grep for:
- `GDRDMA`, `P2P`, `direct pointer`, `busbw`
- `ibv_reg_mr_iova2`, `Bad address`
- `raw VA passthrough`, `GPU device memory mirror failed`

If testing `runsc-rdma`, capture both NCCL logs and the runtime `--debug` log so MR-registration failures can be correlated with NCCL transport selection.
