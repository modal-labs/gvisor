...
Bare metal uses **`sudo bash -c`** with **`ulimit -l unlimited`** (memlock) and **`PYTHONPATH`** pointing at the user `site-packages` tree so `torch`/`torchrun` resolve under root.
