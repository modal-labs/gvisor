...
// setXattrsFromPAXRecords extracts xattrs from hdr.PAXRecords and sets them on
// the inode.
...
func (d *dentry) createTarHeader(path string, inoToPath map[uint64]string) (*tar.Header, error) {
	...
	if xattrs := d.inode.xattrs.RawXattrs(); len(xattrs) > 0 {
		...
		for k, v := range xattrs {
			// PaxRecords require that key and value are non-empty UTF-8 strings
			// and that the key does not contain '='.
