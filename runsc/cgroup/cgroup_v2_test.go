...
func TestInstallUnified(t *testing.T) {
	for _, tc := range []struct {
		name      string
		resources *specs.LinuxResources
		// pre-created files in the fake cgroup.
		seedFiles []string
		// expected file contents after Install()
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			err = cg.Install(tc.resources)
			if tc.wantErrSub != "" {
				if err == nil || !strings.Contains(err.Error(), tc.wantErrSub) {
					t.Fatalf("Install() error = %v, want substring %q", err, tc.wantErrSub)
				}
				return
			}
			if err != nil {
